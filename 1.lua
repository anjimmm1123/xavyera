...
local Tab = Window:CreateTab("Informasi", "Info Player")
local Section = Tab:CreateSection("Instan Fishing")
Section:Set("Auto FISHING")
Rayfield:Notify({
   Title = "Auto FIshing Start",
   Content = "Start",
   Duration = 6.5,
   Image = 4483362458,
})
...
local Tab = Window:CreateTab("Menu", "Main Menu")
local Tab = Window:CreateTab("Auto", "Fishing")
local Tab = Window:CreateTab("Teleport", "Tp")
local Tab = Window:CreateTab("Settings", "Settings")
local Tab = Window:CreateTab("WebHook", "Discord")
local Tab = Window:CreateTab("Misc", "Misc")


Rayfield:LoadConfiguration()    if not readfile or not isfile or not isfile(CONFIG_FILE) then return end
    pcall(function()
        local data = HttpService:JSONDecode(readfile(CONFIG_FILE))
        for k, v in pairs(data) do
            if DefaultConfig[k] ~= nil then Config[k] = v end
